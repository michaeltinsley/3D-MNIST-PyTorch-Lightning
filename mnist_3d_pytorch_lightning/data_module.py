...
class MNIST3DDataset(Dataset):
    def __init__(
        ...
        :param train: Create training dataset if True, test dataset if False (default is True)
    ...
    def get_dataset(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with h5py.File(self.dataset_path, "r") as hf:
            X = hf[f"X_{'train' if self.train else 'test'}"][:]
            y = hf[f"y_{'train' if self.train else 'test'}"][:]
...
class MNIST3DDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        :param validation_split: Amount of training set to use for validation, must be between 0 and 1 (default is 0.1)
        :param quiet: Suppress download progress output (default is False)
        """
        super(MNIST3DDataModule, self).__init__()
